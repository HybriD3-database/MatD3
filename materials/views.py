...
def dataset_author_check(view):
    ...
    def wrap(request, *args, **kwargs):
        ...
        if dataset.created_by == request.user:
            ...
            logger.warning(
                f"Data set was created by {dataset.created_by} but "
...
def staff_status_required(view):
    ...
    def wrap(request, *args, **kwargs):
        if request.user.is_staff:
            ...
            logger.warning(
                f"{request.user.username} is trying to submit data "
...
class SystemView(generic.ListView):
    ...
    def get_queryset(self, **kwargs):
        return (
            models.Dataset.objects.filter(system__pk=self.kwargs["pk"])
            .annotate(
                is_atomic_structure=Case(
                    When(primary_property__name="atomic structure", then=Value(True)),
                    default=Value(False),
                    output_field=BooleanField(),
                )
            )
            .order_by("-is_atomic_structure")
        )

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        system = models.System.objects.get(pk=self.kwargs["pk"])

        # Fetch the elements and stoichiometry
        stoichiometry_entries = models.System_Stoichiometry.objects.filter(
            system=system
        )
        element_dict = {}
        for entry in stoichiometry_entries:
            elements = models.Stoichiometry_Elements.objects.filter(
                system_stoichiometry=entry
            )
            element_dict[entry.stoichiometry] = {
                element.element: element.float_value for element in elements
            }

        context["element_dict"] = element_dict
        return context
...
class PropertyAllEntriesView(generic.ListView):
    ...
    def get_queryset(self, **kwargs):
        return models.Dataset.objects.filter(
            system__pk=self.kwargs["system_pk"]
        ).filter(primary_property__pk=self.kwargs["prop_pk"])
...
class CompareView(generic.ListView):
    ...
    def get_queryset(self, **kwargs):
        return models.Dataset.objects.filter(
            pk__in=[self.kwargs["pk1"], self.kwargs["pk2"]]
        )
...
class SearchFormView(generic.TemplateView):
    ...
    def get(self, request):
        material_ids = models.System.objects.all().values_list("pk", "compound_name")
        dataset_ids = (
            models.Dataset.objects.all()
            .values_list("pk", "system__compound_name", "primary_property__name")
            .order_by("pk")
        )
        return render(
            request,
        ...
        if form.is_valid():
            ...
            if search_term == "formula":
                systems = models.System.objects.filter(
                    ...
                    | Q(iupac__icontains=search_text)
                ...
                systems = models.System.objects.filter(
                    organic__icontains=search_text
                ).order_by("organic")
            elif search_term == "inorganic":
                systems = models.System.objects.filter(
                    inorganic__icontains=search_text
                ).order_by("inorganic")
                ...
                query = reduce(
                    operator.or_,
                    (
                        Q(dataset__reference__authors__last_name__icontains=x)
                        for x in keywords
                    ),
                )
...
class AddDataView(StaffStatusMixin, generic.TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.GET.get("reference"):
            main_form.fields["fixed_reference"].initial = models.Reference.objects.get(
                pk=request.GET.get("reference")
            )
        if request.GET.get("qresp-server-url"):
            ...
            main_form.fields["caption"].initial = paper_detail["charts"][
                qresp_chart_nr
            ]["caption"]
    ...
    def perform_create(self, serializer):
        ...
        for name in self.request.POST:
            if name.startswith("first-name-"):
                ...
                authors = (
                    models.Author.objects.filter(first_name=first_name)
...
def dataset_to_zip(request, dataset):
    ...
    zf.writestr("files/info.txt", utils.dataset_info(dataset, request.get_host()))
    # Main data
    ...
    for file_ in (f.dataset_file.path for f in dataset.files.all()):
        zf.write(file_, os.path.join("files/additional", os.path.basename(file_)))
    zf.close()
...
class DatasetViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def summary(self, request):
        ...
        serializer = serializers.DatasetSerializerSummary(filtered_queryset, many=True)
        return Response(serializer.data)
    ...
    def files(self, request, pk):
        ...
        response = HttpResponse(
            in_memory_object.getvalue(), content_type="application/x-zip-compressed"
        )
...
def submit_data(request):
    ...
    def clean_value(value):
        ...
        if "(" in value:
            ...
            if "." not in error and left_paren_start > len(error):
                error = (
                    re.sub("[1-9]", "0", value[: left_paren_start - len(error)]) + error
                )
    ...
    def add_numerical_value(
        numerical_values, errors, upper_bounds, value, is_secondary=False, counter=0
    ):
    ...
    def add_datapoint_ids(values, n_values, n_datapoints):
        ...
        pks = list(
            models.Datapoint.objects.all()
            .order_by("-pk")[:n_datapoints]
    ...
    def generate_numerical_value_ids(array, model):
        ...
        """
        pks = list(
            models.NumericalValue.objects.all()
            .order_by("-pk")[: len(array)]
            .values_list("pk", flat=True)
        )
        filtered_list = []
        for i, pk in enumerate(reversed(pks)):
            if array[i]:
                filtered_list.append(
                    model(
                        created_by=request.user, numerical_value_id=pk, value=array[i]
                    )
                )
    ...
    def create_input_file(
        dataset, import_file_name, data_as_str, i_subset, multiple_subsets
    ):
        ...
        if data_as_str:
            ...
            dataset.input_files.create(created_by=dataset.created_by, dataset_file=f)
    ...
    dataset.primary_property_label = form.cleaned_data["primary_property_label"]
    if form.cleaned_data["two_axes"]:
        ...
        dataset.secondary_property_label = form.cleaned_data["secondary_property_label"]
    ...
    dataset.representative = not bool(
        models.Dataset.objects.filter(system=dataset.system).filter(
    ...
    if form.cleaned_data["with_synthesis_details"]:
        synthesis = models.SynthesisMethod(created_by=request.user, dataset=dataset)
    ...
    if form.cleaned_data["with_experimental_details"]:
        experimental = models.ExperimentalDetails(
            created_by=request.user, dataset=dataset
        )
        experimental.method = form.cleaned_data["experimental_method"]
        experimental.description = form.cleaned_data["experimental_description"]
        experimental.save()
    ...
    if form.cleaned_data["with_computational_details"]:
        computational = models.ComputationalDetails(
            created_by=request.user, dataset=dataset
        )
        ...
        computational.level_of_relativity = form.cleaned_data["level_of_relativity"]
        computational.basis_set_definition = form.cleaned_data["basis_set_definition"]
        computational.numerical_accuracy = form.cleaned_data["numerical_accuracy"]
        computational.save()
    ...
    if form.cleaned_data["stoichiometry"]:
        ...
        system_stoichiometry = System_Stoichiometry.objects.create(
            system=dataset.system, stoichiometry=stoichiometry_value
        ).save()
    ...
    for i_subset in range(1, int(form.cleaned_data["number_of_subsets"]) + 1):
        ...
        if dataset.primary_property.name == "atomic structure":
            ...
            for line in form.cleaned_data[f"atomic_coordinates_{i_subset}"].split("\n"):
                if form.cleaned_data[f"geometry_format_{i_subset}"] != "aims":
                    break
                try:
                    m = re.match(
                        r"\s*lattice_vector" + 3 * r"\s+(-?\d+(?:\.\d+)?)" + r"\b", line
                    )
                    if m:
                        coords = m.groups()
                        ...
                        datapoints.append(
                            models.Datapoint(created_by=request.user, subset=subset)
                        )
                        symbols.append(
                            models.Symbol(created_by=request.user, value=coord_type)
                        )
                        symbols.append(
                            models.Symbol(
            ...
            # Get kpoints
            k_labels = []
            for line in form.cleaned_data[f"subset_datapoints_{i_subset}"].splitlines():
            ...
            for f in dataset.files.all():
                if re.match(r"band10\d+\.out", os.path.basename(f.dataset_file.name)):
                    files.append(f.dataset_file)
                if os.path.basename(f.dataset_file.name) in [
                    "band_structure_full.png",
                    "band_structure_small.png",
                ]:
                    return error_and_return(
                        form,
                        dataset,
            ...
            try:
                for line in form.cleaned_data[
                    ...
                    add_numerical_value(numerical_values, errors, upper_bounds, y_value)
            except ValueError:
                return error_and_return(
                    form, dataset, f"Could not process line: {line}"
                )
        ...
        for key in form.cleaned_data:
            if key.startswith(f"fixed_property_{i_subset}_"):
                ...
                subset.fixed_values.create(
                    created_by=request.user,
                    counter=counter,
                    physical_property=form.cleaned_data["fixed_property_" + suffix],
    ...
    models.Error.objects.bulk_create(generate_numerical_value_ids(errors, models.Error))
    ...
    if form.cleaned_data["qresp_fetch_url"]:
        paper_detail = requests.get(
            form.cleaned_data["qresp_fetch_url"], verify=False
        ).json()
        download_url = paper_detail["fileServerPath"]
        chart_detail = paper_detail["charts"][form.cleaned_data["qresp_chart_nr"]]
        chart = requests.get(
            f'{download_url}/{chart_detail["imageFile"]}', verify=False
        )
    ...
    message = mark_safe(message + f' <a href="{dataset_url}">View</a> the data set.')
    if form.cleaned_data["qresp_search_url"]:
        messages.success(request, message)
        return redirect(
            "/materials/import-data",
            kwargs={
        ...
        return redirect(f'{form.cleaned_data["return_url"]}?pk={dataset.pk}')
    else:
        messages.success(request, message)
...
def data_for_chart(request, pk):
    ...
    response = {
        "primary-property": dataset.primary_property.name,
    ...
    for subset in dataset.subsets.all():
        ...
        for value in models.NumericalValueFixed.objects.filter(subset=subset):
            fixed_values.append(
                f" {value.physical_property} = " f"{value.formatted()} {value.unit}"
            )
        ...
        values = (
            models.NumericalValue.objects.filter(datapoint__subset=subset)
            .order_by("datapoint_id", "qualifier")
...
def get_subset_values(request, pk):
    ...
    values = (
        models.NumericalValue.objects.filter(datapoint__subset__pk=pk)
        .select_related("error")
        .select_related("upperbound")
        .order_by("qualifier", "datapoint__pk")
    )
...
def get_jsmol_input(request, pk):
    ...
    if dataset.input_files.exists():
        filename = os.path.basename(dataset.input_files.first().dataset_file.path)
        return HttpResponse(
            f"load /media/data_files/dataset_{dataset.pk}/{filename} " "{1 1 1}"
        )
    return HttpResponse()
...
def report_issue(request):
    ...
    if user.is_authenticated:
        body = (
            f"Description:<blockquote>{description}</blockquote>"
...
def prefilled_form(request, pk):
    ...
    if dataset.synthesis.exists():
        ...
        for field in filter(
            lambda field: type(field) is TextField,
...
class MintDoiView(StaffStatusMixin, generic.TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        request.session["MINT_DOI_RETURN_URL"] = request.META.get("HTTP_REFERER")
        ...
        if "FIGSHARE_ACCESS_TOKEN" in request.session:
            ...
            return render(
                request,
                "materials/figshare_client_id.html",
...
def figshare_callback(request):
    ...
    headers = {"Authorization": f'token {request.session["FIGSHARE_ACCESS_TOKEN"]}'}
    dataset = models.Dataset.objects.get(pk=request.session["MINT_DOI_DATASET_PK"])
    title = dataset.caption
    data_location = f"https://{request.get_host()}/materials/dataset/{dataset.pk}"
    data = {
    ...
    if result.status_code >= 400:
        return error_and_return(result)
    messages.success(request, "A DOI was generated and the data set was published.")
    return redirect(request.session["MINT_DOI_RETURN_URL"])
