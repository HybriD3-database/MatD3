...
def parse_formula(formula):
    ...
    formula = re.sub(r"(\(R/S\)-|\b[SR]-)", "", formula)

    # Define separators (excluding '.')
    separators = r"[\•\-\:]"  # Include other separators if needed

    # Tokenize the formula
    token_pattern = r"(\d+(\.\d+)?|[A-Z][a-z]?|[\(\)\[\]\{\}]|" + separators + r")"
    ...
    while i < len(tokens):
        ...
        if re.match(separators, token):
            # Reset current_multiplier
            current_multiplier = Decimal("1")
            i += 1
            # Check for multiplier after separator
            if i < len(tokens) and re.match(r"^\d+(\.\d+)?$", tokens[i]):
                current_multiplier = Decimal(tokens[i])
                i += 1
        elif re.match(r"^\d+(\.\d+)?$", token):
            # Multiplier before a bracket or element
            current_multiplier *= Decimal(token)
            i += 1
        elif token in "([{":
            # Start of a new group
            stack.append({})
            # Push current_multiplier onto multiplier_stack
            ...
            group = stack.pop()
            group_multiplier = multiplier_stack.pop()
            i += 1
            # Check for multiplier after closing bracket
            multiplier_after_bracket = Decimal("1")
            if i < len(tokens) and re.match(r"^\d+(\.\d+)?$", tokens[i]):
                multiplier_after_bracket = Decimal(tokens[i])
                i += 1
            total_multiplier = group_multiplier * multiplier_after_bracket
            current_group = stack[-1]
            for element, count in group.items():
                count *= total_multiplier
                current_group[element] = (
                    current_group.get(element, Decimal("0")) + count
                )
            ...
            if i < len(tokens) and re.match(r"^\d+(\.\d+)?$", tokens[i]):
                ...
                i += 1
            # Multiply count by current_multiplier
            count *= current_multiplier
            current_multiplier = Decimal("1")
            current_group = stack[-1]
            current_group[element] = current_group.get(element, Decimal("0")) + count
        else:
...
def atomic_coordinates_as_json(pk):
    ...
    vectors = (
        ...
        .filter(datapoint__symbols__isnull=True)
        .order_by("datapoint_id", "counter")
    )
    data = {
        "vectors": [
    ...
    symbols = (
        models.Symbol.objects.filter(datapoint__subset=subset)
        .filter(counter=1)
    ...
    coords = (
        models.NumericalValue.objects.filter(datapoint__subset=subset)
        .filter(datapoint__symbols__counter=1)
        .select_related("error")
        .order_by("counter", "datapoint_id")
    )
    tmp = (
        models.Symbol.objects.filter(datapoint__subset=subset)
        .annotate(num=models.models.Count("datapoint__symbols"))
        .filter(num=2)
        .first()
    )
    if tmp:
...
def plot_band_structure(k_labels, files, dataset):
    ...
    for ik in range(len(k_labels_reduced)):
        k_labels_reduced[ik] = re.sub("[Gg](?:amma)?", "Γ", k_labels_reduced[ik])
    ...
    for i_segment in range(len(data_raw)):
        ...
        for i_kpoint in range(n_kpoints[i_segment]):
            min_index = min(
                min_index,
                numpy.searchsorted(data_raw[i_segment][i_kpoint, :], ENERGY_FULL_MIN),
            )
            max_index = max(
                max_index,
                numpy.searchsorted(data_raw[i_segment][i_kpoint, :], ENERGY_FULL_MAX),
            )
        file_.close()
    ...
    for i_segment in range(len(data_raw)):
        ...
        for i_kpoint in range(n_kpoints):
            data[:, k_start + i_kpoint] = data_raw[i_segment][
                i_kpoint, min_index:max_index
            ]
        k_start += n_kpoints
    ...
    for segment_i in segment_locations:
        ax.axvline(x=segment_i, color="black", linestyle="--", lw=HELPER_LINE_WIDTH)
...
def dataset_info(dataset, server):
    ...
    data = io.StringIO()
    data.write(f"Data available at {server}/materials/dataset/{dataset.pk}\n")
    data.write("\n")
    if dataset.reference:
        ref = dataset.reference
        data.write(
            f'Reference: {ref.getAuthorsAsString()} "{ref.title}", '
    ...
    for subset in dataset.subsets.all():
        ...
        if dataset.primary_property.name == "atomic structure":
            ...
            data.write("Initial crystal system: " f"{CS[subset.crystal_system][1]}\n")
            if pt.crystal_system_final:
                data.write(
                    "Final crystal system: " f"{CS[pt.crystal_system_final][1]}\n"
                )
