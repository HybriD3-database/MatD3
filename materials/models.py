...
class Base(models.Model):
    ...
    created_by = models.ForeignKey(
        get_user_model(), related_name=f"{this}_created_by", on_delete=models.PROTECT
    )
    updated_by = models.ForeignKey(
        get_user_model(), related_name=f"{this}_updated_by", on_delete=models.PROTECT
    )
...
class Dataset(Base):
    ...
    space_group_ID = models.ForeignKey(
        SpaceGroup, blank=True, null=True, on_delete=models.PROTECT
    )
    ...
    def save(self, *args, **kwargs):
        if self.representative:
            ...
            ).update(representative=False)
        # Disable email notifications for verification updates
        # UKH
    ...
    def delete(self, *args, **kwargs):
        ...
        if self.files.exists():
            shutil.rmtree(os.path.dirname(self.files.first().dataset_file.path))
        if self.input_files.exists():
            shutil.rmtree(os.path.dirname(self.input_files.first().dataset_file.path))
        if self.representative:
            Dataset.objects.filter(system=self.system).filter(
                primary_property=self.primary_property
            ).exclude(pk=self.pk).update(representative=True)
    ...
    def num_all_entries(self):
        return (
            Dataset.objects.filter(system=self.system)
            .filter(primary_property=self.primary_property)
            .count()
        )
    ...
    def get_all_fixed_temperatures(self):
        ...
        values = []
        for value in NumericalValueFixed.objects.filter(subset__dataset=self).filter(
        ...
        if self.primary_property.name != "atomic structure":
            for file_ in self.files.all():
                if os.path.basename(file_.dataset_file.name) == "geometry.in":
                    return os.path.join(settings.MEDIA_URL, file_.dataset_file.name)
        return ""
...
class Subset(Base):
    ...
    dataset = models.ForeignKey(
        Dataset, on_delete=models.CASCADE, related_name="subsets"
    )
    space_group_ID = models.ForeignKey(
        SpaceGroup, blank=True, null=True, on_delete=models.PROTECT
    )
    ...
    def get_fixed_values(self):
        ...
        for value in values:
            ...
            output.append([value.physical_property.name, value_str, value.unit.label])
        return output

    def get_lattice_constants(self):
        """Return three lattice constants and angles."""
        symbols = (
            Symbol.objects.filter(datapoint__subset=self)
            .annotate(num=models.Count("datapoint__symbols"))
            .filter(num=1)
            .order_by("datapoint_id")
            .values_list("value", flat=True)
        )
        values_float = (
            NumericalValue.objects.filter(datapoint__subset=self)
            .annotate(num=models.Count("datapoint__values"))
            .filter(num=1)
...
class NumericalValueBase(Base):
    ...
    value_type = models.PositiveSmallIntegerField(default=ACCURATE, choices=VALUE_TYPES)
...
class NumericalValue(NumericalValueBase):
    ...
    datapoint = models.ForeignKey(
        Datapoint, on_delete=models.CASCADE, related_name="values"
    )
...
class Symbol(Base):
    ...
    datapoint = models.ForeignKey(
        Datapoint, on_delete=models.CASCADE, related_name="symbols"
    )
...
class ExternalRepository(Base):
    computational_details = models.ForeignKey(
        ComputationalDetails, on_delete=models.CASCADE, related_name="repositories"
    )
...
class Note(Base):
    dataset = models.ForeignKey(Dataset, on_delete=models.CASCADE, related_name="note")
...
def data_file_path(instance, filename):
    return os.path.join("data_files", f"dataset_{instance.dataset.pk}", filename)
...
class InputDataFile(Base):
    ...
    dataset = models.ForeignKey(
        Dataset, on_delete=models.CASCADE, related_name="input_files"
    )
