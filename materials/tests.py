...
User = get_user_model()
dataset_template = models.Dataset(
    visible=True,
    is_figure=False,
...
class ModelsTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        dataset.save()
        dataset.pk = None
        dataset.primary_property = models.Property.objects.get(name="atomic structure")
        dataset.save()
...
class SeleniumTestCase(LiveServerTestCase):
    ...
    def test_properties_and_units(self):
        ...
        S.find_element_by_xpath(
            '//div[@id="new-property-card"]//button[@type="submit"]'
        ).click()
        ...
        S.find_element_by_xpath(
            '//div[@id="new-unit-card"]//button[@type="submit"]'
        ).click()
    ...
    def test_publication_material(self):
        ...
        S.find_element_by_xpath('//div[@id="institution-1"]//input').send_keys(
            "institution 1"
        )
        ...
        S.find_element_by_xpath('//div[@id="institution-2"]//input').send_keys(
            "institution 2"
        )
        ...
        S.find_element_by_xpath(
            '//div[@id="new-reference-card"]//button[@type="submit"]'
        ).click()
        ...
        S.find_element_by_xpath(
            '//div[@id="new-system-card"]//button[@type="submit"]'
        ).click()
        S.find_element_by_xpath(
            '//div[@id="dynamic-messages"]' '//div[contains(text(), "compound")]'
        )
    ...
    def test_normal_property(self):
        ...
        # Set system
        S.execute_script("selectized['select_system'][0].selectize.setValue(1)")
        # General
        ...
        S.find_element_by_id("id_starting_materials").send_keys("starting material")
        S.find_element_by_id("id_product").send_keys("product")
        S.find_element_by_id("id_synthesis_description").send_keys("description")
        ...
        S.find_element_by_id("id_experimental_description").send_keys("description")
        S.find_element_by_id("id_experimental_comment").send_keys("exp comment")
        ...
        S.find_element_by_id("id_level_of_relativity").send_keys("non-relativistic")
        S.find_element_by_id("id_basis_set_definition").send_keys("pseudopotentials")
        S.find_element_by_id("id_numerical_accuracy").send_keys("1 meV/atom")
        S.find_element_by_id("id_computational_comment").send_keys("comp comment")
        # Data
        ...
        S.find_element_by_xpath(
            f'//form[@action="{submit_link}"]//button[@type="submit"]'
        ).click()
        self.assertEqual(models.Dataset.objects.last().space_group, "F222")
        ...
        S.execute_script("selectized['primary_unit'][0].selectize.clear()")
        ...
        S.find_element_by_xpath('//a[starts-with(@href, "/materials/dataset")]').click()
        S.find_element_by_link_text("See all related data").click()
        # View the other data set
        S.find_element_by_xpath(
            f'//button[@data-target="#synthesis-body-{last_pk}"]'
        ).click()
        sleep(0.2)
        S.find_element_by_xpath(
            f'//button[@data-target="#experimental-body-{last_pk}"]'
        ).click()
        sleep(0.2)
        S.find_element_by_xpath(
            f'//button[@data-target="#computational-body-{last_pk}"]'
        ).click()
        for test_string in (
            "starting material",
            "product",
    ...
    def test_phase_transition(self):
        ...
        S.execute_script("selectized['select_system'][0].selectize.setValue(1)")
        # Input data
        S.find_element_by_id("id_phase_transition_space_group_initial_1").send_keys(
            "C1"
        )
        S.find_element_by_id("id_phase_transition_space_group_final_1").send_keys("C2")
        S.find_element_by_id("id_phase_transition_direction_1").send_keys("increasing")
        S.find_element_by_id("id_phase_transition_hysteresis_1").send_keys("none")
        S.find_element_by_id("id_phase_transition_value_1").send_keys("300...301")
        # Create data set
        submit_link = reverse("materials:submit_data")
        S.find_element_by_xpath(
            f'//form[@action="{submit_link}"]//button[@type="submit"]'
