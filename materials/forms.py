...
class AddSystem(forms.ModelForm):
    ...
    class Meta:
        ...
        help_texts = {
            "group": "Please list all possible variations of this compound (besides those entered above), including different uses of parentheses -- this is necessary to make this compound easily searchable."
        }
        widgets = {
            "description": forms.Textarea(attrs={'rows': 4}),
            "group": forms.TextInput(attrs={'placeholder': 'List alternate names, e.g. AEQTPbI4, (AEQT)PbI4, AE4TPbI4, (AE4T)PbI4 ...'})
        }
...
class AddSynthesisMethod(forms.ModelForm):
    all_fields = ('temperature', 'phase', 'comments', 'synthesis_method', 'source', 'data_extraction_method', 'starting_materials', 'remarks', 'product')
    field_order = ('source', 'data_extraction_method', 'temperature', 'phase',  'synthesis_method', 'starting_materials', 'remarks', 'product', 'comments')
    class Meta:
        model = SynthesisMethod
        exclude = ('contributor', 'publication', 'system', 'method', 'specific_method')
        labels = {
        ...
        widgets = {
            "synthesis_method": forms.Textarea(attrs={'rows': 3, 'placeholder': 'e.g. expirimental, solution grown'}),
...
class AddBandStructure(forms.ModelForm):
    all_fields = ('temperature', 'phase', 'method', 'specific_method', 'comments', 'source', 'data_extraction_method', 'band_gap')
    band_structure_files = forms.FileField(widget=forms.ClearableFileInput(attrs={'multiple': True}), required=False)
    control_in_file = forms.FileField(label="\"control.in\" file", required=False)
