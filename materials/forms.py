...
class AutoCharField(forms.CharField):
    ...
    def __init__(self, model=None, field=None, *args, **kwargs):
        if model:
            ...
            max_length = None
        super().__init__(required=False, max_length=max_length, *args, **kwargs)
...
class AddSystemForm(forms.Form):
    ...
    related_systems = forms.CharField(
        ...
        required=False,
        widget=forms.TextInput(attrs={"class": "form-control"}),
        help_text="" "List of System IDs this system is directly derived from",
    )
...
class AddDataForm(forms.Form):
    ...
    select_reference = forms.ModelChoiceField(
        ...
        help_text="" "Select the reference that is associated with the inserted data.",
    )
    ...
    select_system = forms.ModelChoiceField(
        ...
        required=True,
        widget=forms.Select(attrs={"class": "form-control"}),
        help_text="" "Select the system that is associated with the inserted data.",
    )
    ...
    secondary_property_label = AutoCharField(
        model=models.Dataset,
        field="secondary_property_label",
        widget=forms.TextInput(
            attrs={
                "class": "form-control",
                "placeholder": "Leave empty if not required",
    ...
    space_group = AutoCharField(
        ...
        help_text="" "Space group id. Refer to documentation for complete list.",
    )
    ...
    k_point_grid = AutoCharField(
        ...
        help_text="" "Details of the k-point mesh.",
    )
    level_of_relativity = AutoCharField(
        widget=forms.TextInput(
            attrs={
                "class": "form-control",
                "placeholder": "non-relativistic, atomic ZORA with SOC, Koelling-Harmon...",
            }
        ),
        help_text=""
    ...
    numerical_accuracy = AutoCharField(
        widget=forms.TextInput(
            attrs={
                "class": "form-control",
                "placeholder": "SCF tol. 1 meV/atom, Lebedev grids for angular integration...",
            }
        ),
        help_text=""
    ...
    external_repositories = AutoCharField(
        widget=forms.TextInput(
            attrs={
                "class": "form-control",
                "placeholder": "http://dx.doi.org/00.00000/NOMAD/2000.01.30-5 ...",
            }
        ),
        help_text=""
    ...
    number_of_subsets = forms.CharField(
        ...
        widget=forms.NumberInput(
            attrs={"class": "form-control mx-sm-3", "min": "1", "style": "width:8em"}
        ),
        help_text=""
    ...
    import_file_name = forms.CharField(required=False, widget=forms.HiddenInput())
    ...
    subset_label = AutoCharField(
        ...
        model=models.Subset,
        field="label",
        widget=forms.TextInput(attrs={"class": "form-control subset-label-class"}),
        help_text=""
    ...
    subset_datapoints = forms.CharField(
        ...
        widget=forms.Textarea(
            attrs={
                "class": "form-control subset-datapoints",
                "rows": "4",
    ...
    lattice_constant_b = forms.CharField(
        required=False,
        widget=forms.TextInput(attrs={"class": "form-control", "placeholder": "b"}),
    )
    lattice_constant_c = forms.CharField(
        required=False,
        widget=forms.TextInput(attrs={"class": "form-control", "placeholder": "c"}),
    )
    lattice_constant_alpha = forms.CharField(
        label="Angles (deg)",
        required=False,
        widget=forms.TextInput(attrs={"class": "form-control", "placeholder": "α"}),
    )
    lattice_constant_beta = forms.CharField(
        required=False,
        widget=forms.TextInput(attrs={"class": "form-control", "placeholder": "β"}),
    )
    lattice_constant_gamma = forms.CharField(
        required=False,
        widget=forms.TextInput(attrs={"class": "form-control", "placeholder": "γ"}),
    )
    placeholder_ = "# Enter data here in any format\n# that JMol can read"
    ...
    # Qresp related
    qresp_fetch_url = forms.CharField(required=False, widget=forms.HiddenInput())
    qresp_chart_nr = forms.IntegerField(required=False, widget=forms.HiddenInput())
    qresp_search_url = forms.CharField(required=False, widget=forms.HiddenInput())
        ...
        if args:
            for key, value in args[0].items():
                if key.startswith("subset_datapoints_"):
                    ...
                    self.fields[key] = forms.CharField(
                        required=False, widget=forms.HiddenInput(), initial=value
                    )
    ...
    def clean(self):
        ...
        if not all(map(lambda x: x.isdigit(), data.get("related_data_sets").split())):
            self.add_error(
                "related_data_sets", "This must be a list of space separated integers."
            )
    ...
    def get_subset(self):
        ...
        for field in self.fields:
            if field.startswith("subset_datapoints_"):
                ...
                crystal_system = self.fields["crystal_system_" + counter].initial
                import_file_name = self.fields["import_file_name_" + counter].initial
                ...
                results.append(
                    [counter, import_file_name, crystal_system, label, datapoints]
                )
        return results
